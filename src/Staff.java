...
//         remove a production from the system
    ...
    public void solveRequest(Request r) {
        // solve a request
        IMDB imdb = IMDB.getInstance();
//        find if it's a movie/series/actor request
        for(Actor actor : imdb.actors) {
            if(actor.name.equals(r.problemName)) {
                int choice = Options.updateInfoActorChoices();
                switch (choice) {
                    case 1:
                        Update.updateActorName(actor);
                        break;
                    case 2:
                        Update.updateActorBio(actor);
                        break;
                    case 3:
//               display all actor performances and remember the number the user chooses
                        Update.updateActorPerformance(actor);
                        break;
                }
            }
        }
        ...
        for(Production p : imdb.productions) {
            if (p.title.equals(r.problemName)) {
                if (p instanceof Series) {
                    int option = Options.updateInfoSeriesChoices();
                    switch (option) {
                        case 1:
                    ...
                    int option = Options.updateInfoMovieChoices();
                    switch (option) {
                        case 1:
